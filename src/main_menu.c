...
static void Task_NewGameBirchSpeechSub_WaitForBidoof(u8);
static void Task_NewGameBirchSpeech_StartBirchBidoofPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchBidoof(u8);
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tBidoofSpriteId;
    ...
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForBidoof;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForBidoof(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tBidoofSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchBidoofPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchBidoofPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tBidoofSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tBidoofSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tBidoofSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchBidoof;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchBidoof(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tBidoofSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tBidoofSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tBidoofSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateBidoofSprite(u8 x, u8 y)
{
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    bidoofSpriteId = NewGameBirchSpeech_CreateBidoofSprite(100, 0x4B);
    gSprites[bidoofSpriteId].callback = SpriteCB_Null;
    gSprites[bidoofSpriteId].oam.priority = 0;
    gSprites[bidoofSpriteId].invisible = TRUE;
    gTasks[taskId].tBidoofSpriteId = bidoofSpriteId;
    brendanSpriteId = CreateTrainerSprite(FacilityClassToPicIndex(FACILITY_CLASS_RYAN), 120, 60, 0, &gDecompressionBuffer[0]);
...
{
    u8 str[0x20];
    u8 badgeCount = VarGet(VAR_BADGES_COLLECTED);
